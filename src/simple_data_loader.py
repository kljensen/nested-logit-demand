...
import numpy as np
import pandas as pd
import pyblp  # type: ignore[import-untyped]


def load_nevo_data_simple() -> pd.DataFrame:
    ...
    df = df.merge(
        market_shares_sum.rename("inside_share_total"), left_on="market_ids", right_index=True
    )
    ...
    df = df.merge(
        nest_shares.rename("nest_share"), left_on=["market_ids", "nest_id"], right_index=True
    )
